...
| BE-05 | PARTIAL | Cross-project non-member access is denied for artifacts, Markdown export, workflow run history, audit events, model invocations, external call logs, code skeleton generation, code generation cancellation, code generation retry, and failed task retry in `ProjectControllerTest`. |
