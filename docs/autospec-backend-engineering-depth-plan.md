...
| BE-03 | PARTIAL | Code skeleton generation persists `code_generation_job.status = FAILED`, `error_message`, and `completed_at` when generation fails, and avoids writing an `export_file`; `GET /api/projects/{projectId}/code-generation-jobs` exposes durable job state; `POST /api/projects/{projectId}/code-generation-jobs/{jobId}/cancel` moves RUNNING jobs to `CANCELLED` with `cancelled_at` and `completed_at`; covered by `CodeSkeletonServiceTest` and `ProjectControllerTest`. |
...
| BE-05 | PARTIAL | Cross-project non-member access is denied for artifacts, Markdown export, workflow run history, model invocations, external call logs, code skeleton generation, code generation cancellation, and failed task retry in `ProjectControllerTest`. |
...
| BE-08 | PARTIAL | Flyway migration coverage for `workflow_run`, `audit_event`, `external_call_log`, and `code_generation_job.cancelled_at` in `SchemaInitSqlTest`; `HttpAgentEngineClientTest` uses a local HTTP server stub to verify V4 Agent Engine request/response mapping without external network access. |
